...
* `alert("Nikhil");` — Displays a popup with the text “Nikhil”.

```html
<script>alert("Nikhil");</script>
```
...
# 6. Vulnerable Example: `xss-6.php`
```php
xss-6.php
```
```php
...
* `htmlentities()` converts special characters like `<`, `>`, `"`, `'` into their HTML entities, preventing browsers from executing any injected code.

---

### Why the Code Is Typically Not Vulnerable (Due to `htmlentities()`):

* **Output Encoding:** Script tags or injected code become harmless text.
* **No Active Script Execution:** Prevents reflected XSS by escaping dangerous characters.
* **UTF-8 Support + Flags:** Though missing `ENT_QUOTES | ENT_HTML5` and explicit `UTF-8` charset in the original, it still protects against most common XSS cases.

---

###  Potential Limitations:

* If output context changes (like inside `<script>` or attribute values), `htmlentities()` isn't enough.
* Pre-processing filters or encodings may interfere with `htmlentities()`.
* Missing `charset` declaration could affect encoding accuracy (rare in modern UTF-8 setups).
* Advanced XSS payloads may attempt bypasses in edge-case scenarios.

---

### Summary:

* This code **is not vulnerable** to standard reflected XSS because it safely encodes output with `htmlentities()`.
* To create a **deliberately vulnerable lab**, remove or bypass `htmlentities()`.

---

### Further Reading and Context:

* XSS occurs when **untrusted user input** is reflected in responses **without encoding**.
* Use `htmlspecialchars()` or `htmlentities()` based on context to neutralize threats.
* **Context-aware encoding** is critical — HTML body, attributes, and JavaScript require different encoders.
* Don't rely on blacklists or keyword filters (e.g., blocking "script").

Resources:

* OWASP: Cross Site Scripting (XSS)
* PortSwigger: XSS Explained
* Brightsec’s PHP XSS Guide

---

# 7. Vulnerable Example: `xss-7.php` (File Upload Lab)
```php
xss-7.php
```
```php
<?php
// Display POST data and uploaded file information for debugging
print_r($_POST);
print_r($_FILES);
...
if (isset($_POST['upload'])) {
    ...
    // WARNING:
    // Using original filename without sanitization — vulnerable to path traversal and overwriting.
    // move_uploaded_file() allows uploading any file type — no MIME type or extension checks.
...
* **No Filename Sanitization:** Allows upload of malicious files with `.php`, `.html`, or script extensions.
* **Unrestricted File Types:** No checks on MIME type or file extension.
* **Path Traversal Risk:** Using user-controlled filenames may allow `../` type attacks (if not properly handled).
* **Potential XSS or RCE:** Uploaded HTML or JS files could be accessed via browser if stored in a public directory.
