...
Here is your **cleaned, corrected, and fully structured PHP Filesystem Functions Guide** in professional Markdown format, preserving all original information and fixing errors in both code and text:

---

# 📂 PHP Filesystem Functions Guide

Filesystem functions are built-in PHP functions used to work with files and directories on a server. They allow you to **read, write, copy, move, delete**, and manage file/folder information.

🔗 [View full list on W3Schools](https://www.w3schools.com/php/php_ref_filesystem.asp)

---

## 🧠 Why Use Filesystem Functions?
...
## 📚 Common PHP Filesystem Functions
...
| `unlink()`                                   | Deletes a file                              | `unlink("file.txt");`                    |
| `copy()`                                     | Copies a file                               | `copy("file1.txt", "file2.txt");`        |
| `rename()`                                   | Renames or moves a file/directory           | `rename("old.txt", "new.txt");`          |
| `file_exists()`                              | Checks if a file or directory exists        | `file_exists("file.txt");`               |
| `filesize()`                                 | Gets file size in bytes                     | `echo filesize("file.txt");`             |
| `filetype()`                                 | Returns type (file or dir)                  | `echo filetype("file.txt");`             |
| `realpath()`                                 | Gets absolute path                          | `echo realpath("file.txt");`             |
| `pathinfo()`                                 | Returns file info (dirname, basename, etc.) | `print_r(pathinfo("file.txt"));`         |
| `basename()`                                 | Returns filename                            | `echo basename("file.txt");`             |
| `dirname()`                                  | Returns directory path                      | `echo dirname("file.txt");`              |
...
✅ Always check if files exist using `file_exists()`
✅ Use absolute paths with `realpath()`
⚠️ Be cautious with `unlink()` — deletion is permanent
🔐 Ensure proper file permissions
🧰 Use `fopen()`/`fclose()` for low-level control
...
## 🔧 Full Example: `filesystem_functions.php`
```
filesystem_functions.php
```
```php
...
## 📄 Second Example: `filesystem-functions2.php`
...
* Always use `file_exists()` before file operations.
* Use `unlink()` for deleting, not `delete()` (which doesn't exist in PHP).
* Test file scripts on dev servers to avoid accidental data loss.
* Use `realpath()` and sanitize user input to avoid security risks like path traversal.
* Set correct permissions (`chmod`) and ownership (`chown`) on production servers.
